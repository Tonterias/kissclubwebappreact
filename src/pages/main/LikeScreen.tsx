        ...
        const unsubscribeUser = onSnapshot(userRef, (userSnap) => {
            if (userSnap.exists()) {
                ...
                const unsubscribeParty = onSnapshot(partyRef, async (partySnap) => {
                    if (partySnap.exists()) {
                        const likedArr: string[] = partySnap.data().likedUser || [];
                        likedArr.forEach((likedUser) => {
                            if (activeListeners.current.has(likedUser)) {
                                return;
                            }


                            const likedUserRef = doc(db, "users", likedUser);
                            const unsubscribeLikedUser = onSnapshot(likedUserRef, async (snap) => {
                                ...
                                if (!blocked.includes(currentUserId) && !fetchedUserCache.current.has(likedUser)) {
                                    await fetchLikedUsersData([likedUser], likedUserData.partyId);
                                    fetchedUserCache.current.add(likedUser);
                                }

                            });

                            activeListeners.current.set(likedUser, unsubscribeLikedUser);
                        });
